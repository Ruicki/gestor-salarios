...
import { updateCategoryLimit, toggleCategoryRollover } from '@/app/actions/budget';
...
function BudgetRow({ category, spent, currency }: { category: ExtendedCategory, spent: number, currency: string }) {
    ...
    const [limit, setLimit] = useState(category.monthlyLimit || 0);
    ...
    async function handleSave() {
        if (limit === limitNum) { setIsEditing(false); return; }

        setIsLoading(true);
        const res = await updateCategoryLimit(category.id, Number(limit));
        setIsLoading(false);
                ...
                {isEditing ? (
                    <div className="flex items-center justify-end gap-2">
                        <input
                            type="number"
                            value={limit}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setLimit(parseFloat(e.target.value) || 0)}
                    ...
                    <div
                        onClick={() => { setLimit(limitNum); setIsEditing(true); }}
