...
export default function BudgetPage() {
    ...
    const totalExpenses = activeProfile?.expenses.reduce((sum, exp) => sum + Number(exp.amount), 0) || 0;
    const totalGoals = activeProfile?.goals.reduce((sum, g) => sum + Number(g.currentAmount), 0) || 0;
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="bg-linear-to-br from-zinc-800 to-zinc-900 p-6 rounded-3xl border border-zinc-700/50">
                                ...
                                <p className="text-4xl font-black text-white mt-2">${Number(totalExpenses).toFixed(2)}</p>
                            </div>
                            <div className="bg-linear-to-br from-cyan-900/30 to-zinc-900 p-6 rounded-3xl border border-cyan-800/30">
                                ...
                                <p className="text-4xl font-black text-white mt-2">${Number(totalGoals).toFixed(2)}</p>
                            </div>
                        </div>
                                ...
                                {activeProfile.expenses.map((expense) => (
                                    <div key={expense.id} className="group flex items-center justify-between p-4 bg-zinc-900/30 hover:bg-zinc-800 border border-zinc-800/50 rounded-2xl transition-all">
                                        ...
                                        <div className="flex items-center gap-6">
                                            <span className="font-bold text-zinc-300">${Number(expense.amount).toFixed(2)}</span>
                                            <button
